import { Body, Controller, Delete, Get, HttpCode, Param, Post, Req, UploadedFiles, UseFilters, UseGuards, UseInterceptors } from '@nestjs/common';
...
@ApiExtraModels(
    UploadDocumentResponseDTO, GetChatResponseDTO,
    GetChatsResponseDTO, SendMessageResponseDTO
)
@Controller('chats')
export class ChatController {
        ...
        schema: {
            ...
            properties: {
                files: {
                    ...
                    description: 'PDF or DOCX files to upload (max 3 files, 3MB each)',
                },
                message: {
                    type: 'string',
                    description: 'Optional message - used as chat title when creating new chat (first 16 characters)',
                },
            },
            required: ['files'],
        },
    ...
    @ApiBadRequestResponse({
        description: 'Invalid request - file validation, chat not found, or no files',
        schema: {
            type: 'object',
            properties: {
    ...
    @ApiParam({
        name: 'chatId',
        description: 'Chat ID to send message to, or "new" to create a new chat',
        schema: {
            oneOf: [

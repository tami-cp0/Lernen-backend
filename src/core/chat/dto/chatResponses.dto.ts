...
export class ChatDTO implements InferSelectModel<typeof chats> {
	...
	@ApiProperty({
		example: 'My Document Chat',
		description: 'Chat title (max 16 characters)',
		maxLength: 16,
	})
	title: string;
...
export class ChatMessageDTO implements InferSelectModel<typeof chatMessages> {
	...
	@ApiProperty({
		example: true,
		description: 'Whether the message was marked as helpful by the user',
		required: false,
	})
	...
	@ApiProperty({
		example: 150,
		description: 'Total tokens used in this message turn',
	})
	totalTokens: number;
...
export class GetChatResponseDTO {
	...
	@ApiProperty({
		type: ChatWithMessagesAndDocuments,
		description: 'Chat data with messages and documents',
	})
...
export class GetChatsResponseDTO {
	...
	@ApiProperty({
		type: [ChatDTO],
		description:
			'Array of user chats ordered by creation date (newest first)',
	})
	data: ChatDTO[];
}
